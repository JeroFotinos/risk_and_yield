...
class CropModel:
    ...
    def evolve(self) -> Results:
        ...
        * It keeps the core water balance in n layers, canopy cover dynamics,
        ...
        dds = np.copy(s.dds0)
        # Canopy cover at t=0
        cover[:, :, 0] = self._get_init_cover_t0(
            dds, s.crop_mask, scheme="fill_ones"
        )
        # Initial layer water distribution
        au[:, :, :, 0] = self._get_init_au_layers_t0(
            s.water0, s.aut, s.crop_mask
        )
        # Initialize roots based on dds0
        root[:, :, 0] = self._initial_root_lengths(dds, s.crop_mask, cp)
        # Roots are being initialized to 0 inspite of having dds0 > 0 (!)
        ...
        for t in range(T):
            ...
            # ------------------------ Water Dynamics ------------------------
            # carry yesterday's state into today's slot so we can update
            # in-place
            if t > 0:
                au[:, :, :, t] = au[:, :, :, t - 1]

            # water fraction before updating for the day (used for DD90 &
            # evaporation)
            # Accessible capacity multiplier
            # (e.g., 1 + I(root>500) + I(root>1000) + ...)
            accessible_mult = 1.0
            # sum_layers tiene el agua total del pixel ij, sumando sólo las
            # capas accesibles
            ...
            for k in range(1, L):
                ...
                # por las plantas en ese pixel, de acuerdo al largo de sus
                # raíces
            cap_accessible = (
            ...
            # aut es el agua accesible máxima *por capa*, por el cual
            # multiplicamos a accessible_mult que me dice cuantas capas son
            # accesibles por las plantas en cada pixel
            with np.errstate(invalid="ignore", divide="ignore"):
                # el porcentaje de agua útil para las plantas en el pixel ij
                # es el agua total accessible por las plantas `sum_layers`,
                # dividido el máximo posible `cap_accessible`
                p_au_now = np.clip(
            ...
            # This is the correction for RUE due to water stress
            # (RUE_{Act} = RUE_{Pot} * T°EUR * CEHR)
            ceh_r[:, :, t] = (
                self.stress_sigmoid(
            ...
            # mask for stressed, yet not null efficiency, due to
            # LOW temperatures -- we “select” those values
            sel = (ti > cp.tbr) & (ti < cp.tor1)
            # for those, the efficiency is the fraction of departure from the
            # minimal viable temperature, with respect to the maximum
            # departure.
            # (Below tbr the efficiency is 0, and above tor1 it is 1, so this
            # is a linear interpolation between those.)
            th[sel] = (ti[sel] - cp.tbr) / (cp.tor1 - cp.tbr + 1e-9)
            # Now we select stress, yet not null efficiency points, due to
            # HIGH temperatures
            ...
            # We calculate the new canopy cover, and we take either that, or
            # the stationary canopy cover c_fin
            ...
            # We add a day to the list of days with available water percentage
            # below 0.9
            DD90 = DD90 + 1
            ...
            # Per-layer mask: for layer k, it's 1 where k < n_accessible,
            # else 0.
            # Shape: (H, W, L)
            mask_layers = np.stack(
                [(n_accessible > k) for k in range(L)], axis=2
            )

            # Per-pixel share = transp / n_accessible;
            # broadcast to layers, then mask
            ...
            # For this, we'll calculate the sum of the available water across
            # layers
            sum_layers = au[
                :, :, 0, t
            ]  # the initial layer always adds to the total
            ...
            # As the per-layer capacity aut is
            # soil_layer_depth_mm * (cc - pmp),
            # and all layers have the same thickness/depth equal to
            # soil_layer_depth_mm, we can simply multiply by the number of
            # accessible layers to get the total depth
            # soil_layer_depth_mm * accessible_mult, thus
            # obtaining the capacity accessible to plants as
            # cap_accessible =
            # = accessible_mult * soil_layer_depth_mm * (cc - pmp),
            ...
            bi[:, :, t] = np.maximum(
                0.0, cover[:, :, t] * w.par[t] * eur_act[:, :, t]
            )
            # Cumulative biomass
            bt[:, :, t] = (
                bi[:, :, t] if t == 0 else bt[:, :, t - 1] + bi[:, :, t]
            )
            # Yield
            rend[:, :, t] = (
                bt[:, :, t] * ici * ceh_r[:, :, t] * cp.harvest_index
            )
